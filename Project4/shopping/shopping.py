...
def load_data(filename):
    ...
    revenue_list_int=[1 if x=='TRUE' else 0 for x in revenue_list]
    ans_list=[]
    ...
    for index,row in df.iterrows():
        ans_list.append(row.tolist())

    ans=(ans_list,revenue_list_int)
    return ans
...
def evaluate(labels, predictions):
    ...
    true_positive=df[(df['labels']==1) & (df['predictions']==1)].shape[0]
    true_negative=df[(df['labels']==0) & (df['predictions']==0)].shape[0]
    false_positive=df[(df['labels']==0) & (df['predictions']==1)].shape[0]
    false_negative=df[(df['labels']==1) & (df['predictions']==0)].shape[0]
